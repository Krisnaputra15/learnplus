...
class teacherController extends Controller
{
    public function dashboard(){
        $profile = teacher::where('id',Session::get('id_user'))->first();
        $classcount = classes::where('id_guru',$profile->id)->count();
        return view('teacher.dashboard',compact('profile','classcount'));
    }
    public function editprofile($id){
        $profile = teacher::where('id',$id)->first();
        $classcount = classes::where('id_guru',$profile->id)->count();
        return view('teacher.editprofile',compact('profile','classcount'));
    }
        ...
        if($r->hasFile('picture')){
            if($r->old_password != null && $teacher->password == md5($r->old_password)){
                if($r->new_password != $r->password_confirmation){
                    ...
                    return redirect('/teacher-'.$r->id_user.'-edit');
                }
                $file = $r->file('picture');
    ...
    public function home(){
        ...
        $forcount = classes::where('id_guru',Session::get('id_user'))->count(); 
        if($forcount < 2 || $forcount == 1){
            $classes2 = classes::where('id_guru',Session::get('id_user'))->first(); 
        }
        else{
            $classes2 = classes::where('id_guru',Session::get('id_user'))->get(); 
        }
        return view('teacher.home',compact('classes','classes2','forcount'));
    }
    public function makeclass(request $r){
        return view('teacher.makeclass');
    }
    public function makeclassprocess(request $r){
        ...
        if($r->hasFile('picture')){
            $file = $r->file('picture');
            $storage = public_path('keperluan_web/sampul_kelas/id_guru_'.$r->id_guru);
            ...
            $storage = './public/keperluan_web/sampul_kelas/id_guru_'.$r->id_guru.'';
            if(!File::isDirectory($storage)){
                File::makeDirectory($storage, 077, true, true, true);
            }
            $classes = classes::create([
    ...
    public function editclass($id){
        ...
        return view('teacher.editclass',compact('class'));
    }
    ...
    public function viewtopic($id){
        ...
        return view('teacher.topic',compact('forcount','classes2','class'));
    }
    public function maketopic($id){
        $class = classes::where('id',$id)->first();
        return view('teacher.maketopic',compact('class'));
    }
    public function maketopicprocess(request $r){
        ...
        if($validator->fails()){
            Session::flash('alert_warning','Form uncompletely filled');
            return redirect('/class-'.$r->id_kelas.'-make-topic');
        }
        ...
        if($check != null){
            ...
            if($r->hasFile('nama_file')){
                try{
                    ...
                    return redirect('/class-'.$r->id_kelas.'-topics');
                    }
                    catch(\exception $e){
    ...
    public function viewtopicdetail($id){
        ...
        $filesubmitcount = DB::table('attachment_murid')->join('data_topik','data_topik.id_topik','attachment_murid.id_topik')
                           ->where('attachment_murid.id_topik','=',$topic->id,'and','data_topik.id_topik','=',$topic->id,'and','data_topik.status','like','%completed%')
                           ->count();
        $filesubmit='';
        if($filesubmitcount < 2 && $filesubmitcount == 1){
        ...
        if($countdatatopikselesai > 1){
            $datatopikselesai = db::table('data_topik')->join('students','data_topik.id_murid','=','students.id')
                                ->where('data_topik.id_topik','=',$id,'and','status','like','%completed%')
        ...
        if($countdatatopikbelum > 1){
            $datatopikbelum = db::table('data_topik')->join('students','data_topik.id_murid','=','students.id')
                                ->where('data_topik.id_topik','=',$id,'and','status','like',"%uncompleted%")
                                ->select('data_topik.*','students.*')
                                ->get();
        }
    ...
    public function edittopic($id){
        ...
        */
        return view('teacher.edittopic',compact('topic'));
    }
    public function edittopicprocess(request $r){
        ...
        if($r->hasFile('nama_file')){
            ...
            return redirect('/topic-'.$r->id_topik.'-detail');
        }
        else{
            $update = topics::where('id',$r->id_topik)->update([
                'id_kelas' => $r->id_kelas,
    ...
    public function deletetopic($id){
        ...
        if($attachmentcount > 0){
            ...
            if($delete){
                ...
                return redirect('/class-'.$class->id_kelas.'-topics');
            }
            else{
